...
use gpui::{
    ...
    Result, SharedString, Subscription, Timer, TitlebarOptions, Window, WindowBounds, WindowOptions,
    actions, div, ease_in_out, prelude::*, px, rgb, size,
};
...
impl AngryHub {
    fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let prs = cx.new(|cx| {
            PullRequestsView::new(session.clone(), router.clone(), window, cx)
        });
        let activity = cx.new(|cx| {
            ActivityView::new(session.clone(), router.clone(), window, cx)
        });
        ...
        view._subscriptions.push(cx.observe(&view.session, |this, _, cx| {
