...
impl HelloWorld {
    fn new(auth: AuthState, window: &mut Window, cx: &mut Context<Self>) -> Self {
        let mut view = Self {
            auth,
            view: AppView::PullRequests,
            previous_view: AppView::PullRequests,
            ...
            activity_fetch_in_flight: false,
    ...
    fn fetch_activity(&mut self, cx: &mut Context<Self>) {
        if self.activity_fetch_in_flight {
            return;
        }
        self.activity_fetch_in_flight = true;
        let host = code_host();
        cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                ...
                .await;
            this.update(cx, |this, _cx| {
                this.activity_fetch_in_flight = false;
    ...
    fn select_view(&mut self, view: AppView, cx: &mut Context<Self>) {
        if self.view != view {
...
impl Render for HelloWorld {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut content = match &self.auth {
            ...
            AuthState::LoggedIn { prs } => match self.view {
                AppView::Activity => match &self.activity {
                    ...
                    ActivityState::Failed(e) => vec![
                        ...
                        Button::new("retry-activity", "Retry")
                            .on_click(cx.listener(|state, _, _, cx| state.fetch_activity(cx)))
                            .into_any_element(),
                    ],
                ...
                AppView::PullRequests => match prs {
                        ...
                        .child(
                            div()
                                ...
                                .when(
                                    ...
                                    ) && self.view == AppView::PullRequests)
                                        || (matches!(self.activity, ActivityState::Loading)
                                            && self.view == AppView::Activity),
                                    |this| this.flex_1(),
                                )
                                .when(
                                    !((matches!(
                                    ...
                                    div()
                                        ...
                                        .child(
                                            SegmentedControl::new("main-view")
                                                ...
                                                .on_change(cx.listener(|state, id, _, cx| {
                                                    state.select_view(
                                                        if id == "activity" {
                                                            AppView::Activity
                                                        } else {
                                                            AppView::PullRequests
                                                        },
                                                        cx,
                                                    );
                                                })),
